...
public class CosStatsSetIntProcedure {
		...
		try {
			for (Entity entityiterator : EntityArgument.getEntities(arguments, "name")) {
				...
				if ((new Object() {
					...
					}.getEntity()).getDisplayName().getString() + "'s Int stat have been set to " + ((new Object() {
						public Entity getEntity() {
							try {
								return EntityArgument.getEntity(arguments, "name");
							} catch (CommandSyntaxException e) {
					...
					}.getEntity()).getCapability(ChildrenOfSilenceModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ChildrenOfSilenceModVariables.PlayerVariables())).StatInt)), false);
			}
		} catch (CommandSyntaxException e) {
			e.printStackTrace();
		}
