...
public class CosStatsSetConProcedure {
		...
		try {
			for (Entity entityiterator : EntityArgument.getEntities(arguments, "name")) {
				...
				if ((new Object() {
					...
					}.getEntity()).getDisplayName().getString() + "'s Con stat have been set to " + ((new Object() {
						public Entity getEntity() {
							try {
								return EntityArgument.getEntity(arguments, "name");
							} catch (CommandSyntaxException e) {
					...
					}.getEntity()).getCapability(ChildrenOfSilenceModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ChildrenOfSilenceModVariables.PlayerVariables())).StatCon)), false);
			}
		} catch (CommandSyntaxException e) {
			e.printStackTrace();
		}
